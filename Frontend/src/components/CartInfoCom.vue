...
<template>
    <div class="bg-gray-100 overflow-y-hidden" style="height: 700px">
            ...
            <div class="w-full absolute z-10 right-0 h-full overflow-x-hidden transform translate-x-0 transition ease-in-out duration-700" id="checkout">
                <div class="flex items-end lg:flex-row flex-col justify-end" id="cart">
                    <div class="lg:w-1/2 md:w-8/12 w-full lg:px-8 lg:py-14 md:px-6 px-4 md:py-8 py-4 bg-white overflow-y-auto overflow-x-hidden lg:h-screen h-auto" id="scroll">
                        ...
                        <p class="lg:text-4xl text-3xl font-black leading-10 text-gray-800 pt-3">Ordered goods</p>
                        <div v-for="cartItem in cartItems" :key="cartItem.id"  class="md:flex items-strech py-8 md:py-10 lg:py-8 border-t border-gray-50">
                            <div  class="md:w-4/12 2xl:w-1/4 w-full">
                                <img :src="cartItem.imageUrl" alt="Black Leather Bag" class="h-full object-center object-cover md:block hidden" />
                                <img :src="cartItem.imageUrl" alt="Black Leather Bag" class="md:hidden w-full h-full object-center object-cover" />
                            </div>
                            <div class="md:pl-3 md:w-8/12 2xl:w-3/4 flex flex-col justify-center">
                                <p class="text-xs leading-3 text-gray-800 md:pt-0 pt-4">{{cartItem.id}}</p>
                                <div class="flex items-center justify-between w-full pt-1">
                                    <p class="text-base font-black leading-none text-gray-800">{{cartItem.name}}</p>
                                    <select aria-label="Select quantity" class="py-2 px-1 border border-gray-200 mr-6 focus:outline-none" >
                                            <option v-for="n in Array.from(Array(Number(cartItem.quantity)).keys(), (i) => i + 1)" :key="n.quantity" :value="n">{{n}}</option>
                                          
                                        </select>
                                </div>
                                <p class="text-xs leading-3 text-gray-600 pt-2">Size: {{cartItem.size}}</p>
                                <p class="text-xs leading-3 text-gray-600 py-4">Color : {{cartItem.color}}</p>
                               
                                <div class="flex items-center justify-between pt-5">
                                    <div class="flex itemms-center">
                                        <p class="text-xs leading-3 underline text-gray-800 cursor-pointer">Add to favorites</p>
                                        <p class="text-xs leading-3 
                                        underline text-red-500 pl-5 cursor-pointer" 
                                        @click="removeFromCart"
                                        >Remove</p>
                                    </div>
                                    <p class="text-base font-black leading-none text-gray-800">${{cartItem.price}}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex flex-col lg:h-screen h-auto lg:px-8 md:px-7 px-4 lg:py-20 md:py-10 py-6 justify-between overflow-y-auto">
                            <div>
                                ...
                                <div class="flex items-center justify-between pt-5">
                                    ...
                                    <p class="text-base leading-none text-gray-800">${{totalShipping}}</p>
                                </div>
                                <div class="flex items-center justify-between pt-5">
                                    <p class="text-base leading-none text-gray-800">Tax</p>
                                    <p class="text-base leading-none text-gray-800">${{totalTax}}</p>
                                </div>
                            </div>
                            <div>
                                <div class="flex items-center pb-6 justify-between lg:pt-5 pt-20">
                                    <p class="text-2xl leading-normal text-gray-800">Total</p>
                                    <p class="text-2xl font-bold leading-normal text-right text-gray-800">$ {{totalTotal(totalPrice,totalShipping,totalTax)}} </p>
                                </div>
...
import axios from 'axios';
  export default {
    ...
    computed:{
      ...
      totalTax(){
        return this.cartItems.reduce((sum, item)=> sum + Number(item.tax), 0); 
      },
      totalShipping(){
        return this.cartItems.reduce((sum, item)=> sum + Number(item.shipping), 0); 
      },
    ...
    methods: {
        ...
        totalTotal(totalPrice, totalShipping, totalTax){
          var totalSum = 0;
          totalSum = totalPrice + totalShipping + totalTax;
